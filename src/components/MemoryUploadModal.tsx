...
import { addMemory } from '../services/firestore';
import {
  uploadVideo,
  uploadThumbnail,
...
export function MemoryUploadModal({ isOpen, onClose, people, onSuccess }: MemoryUploadModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        language === 'ar'
          ? 'فشل تحميل الذكرى. يرجى المحاولة مرة أخرى'
          ...
          <div className="p-8 md:p-10">
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-3xl font-bold text-text">
                {language === 'ar' ? 'إضافة ذكرية جديدة' : 'Add New Memory'}
              </h2>
              {!isUploading && (
                <button
                  onClick={handleClose}
